...
export const controlBlocks: ThemeBlockPackSource = {
  ...
  files: {
    ...
    "blocks/toggle.html": `<button{{ attrs }} class="action toggle-control" type="button" aria-pressed="{{ attr:pressed:false }}"{{ optional:action:data-action }}{{ optional:target:data-target }}{{ optional:target:aria-controls }}{{ bool:disabled }}>{{ attr:label }}</button>`,
