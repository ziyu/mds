...
| Alert | Equivalent | Shared `callout`; Rich also provides `note`, `info`, `warning`, `danger`, and `success` aliases |
...
| Attachment | Existing | Rich `attachment` with media, description, actions, state, and native download metadata |
| Avatar | Existing | `avatar` |
| Badge | Existing | `badge` |
| Breadcrumb | Existing | `breadcrumb` with `breadcrumb-item` children |
| Bubble | Existing | Rich `bubble` with variant, alignment, and reactions slot |
...
| Chart | Existing | Rich `chart`, `chart-series`, and `chart-point` use native meter fallbacks; `!chart` remains available for external chart media |
...
| Data Table | Existing | Rich `data-table`, `data-column`, `data-row`, and `data-cell` provide filtering, sorting, pagination, optional row selection, and a native table fallback |
...
| Dropdown Menu | Existing | Shared `dropdown`, `menu`, and `menu-item`; Rich retains the `dropdown-menu` compatibility alias |
...
| Marker | Existing | Rich `marker` with default, border, and separator variants plus status semantics |
| Menubar | Existing | `menubar` composes dropdown/menu blocks with arrow-key navigation and mutual exclusion |
| Message | Existing | Rich `message` with avatar, header, body, footer, sender, status, and alignment semantics |
| Message Scroller | Existing | Rich `message-scroller` provides a focusable live transcript, follow-latest behavior, and jump-to-latest control |
...
| Table | Equivalent | Markdown tables; richer behavior belongs to the Rich data-table contract |
| Tabs | Existing | `tabs` |
...
- Data tables and composable chart data are implemented by `@mds-crate/theme-rich`.
- Attachment, bubble, marker, message, and message scroller are implemented by `@mds-crate/theme-rich`.
