/* Folio theme progressive enhancement. */
(() => {
  ...
  for (const tabs of document.querySelectorAll(".tabs")) {
    ...
    panels.forEach((panel, index) => {
      const name = panel.getAttribute("data-slot") || `Part ${index + 1}`;
      const panelId = panel.id || `folio-tab-panel-${Math.random().toString(36).slice(2)}`;
